 ...
 *
 */
public class Duffing2 extends javax.swing.JPanel
{
	...
	private double x0 = 0, xf = 2.0;
	private double y0 = 0, yf = 2.5;
	...
	private void getSolution()
	{
		...
		w = new double [501];
		Q = new double [3][w.length];
		for (int i = 0; i < w.length; ++i) {
			// evaluate the new value of w
			w[i] = i * 0.004;
			param [4] = w[i];
			double [] v = {0.1, 0.11};
			...
			// now define Q for g = 10
			Q[0][i] = qValue(x, param);
			
			
			// solve the equation for g = 10
			v = new double [] {0.1, 0.11};
			x = new double [n];
			x[0] = v[0];
			param[6] = 100.0;
			for (int j = 1; j < x.length; ++j) {
	...
	public void paintComponent (java.awt.Graphics g) {
		...
		for (int i = 1; i < w.length; ++i) {
			g.drawLine(toPx(w[i - 1]), toPy(Q[0][i - 1]), toPx(w[i]), toPy(Q[0][i]));
			g.drawLine(toPx(w[i - 1]), toPy(Q[1][i - 1]), toPx(w[i]), toPy(Q[1][i]));
			g.drawLine(toPx(w[i - 1]), toPy(Q[2][i - 1]), toPx(w[i]), toPy(Q[2][i]));
		}
	}
