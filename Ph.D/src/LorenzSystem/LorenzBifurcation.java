...
public class LorenzBifurcation extends JPanel 
{
	// instance variables
	private static double y0 = 0.0;
	private static double yf = 400.0;
	private static double x0 = 0.0;
	private static double xf = 325.0;
	...
	private double [] getMaxima (double [] v0)
	{
		...
		for (int i = 0; i < iterate; ) {
			...
			if (Math.abs(v1[2] - v0[2]) < 0.0001 &&
					Math.abs(v1[2] - v2[2]) < 0.0001)
				return new double [] {v1[2]};
		...
		SwingUtilities.invokeLater (new Runnable () {
			public void run () {
				...
				JFrame frame = new JFrame (
						"Bifurcation Diagram of Lorenz Tent Map");
				frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
				frame.add(new LorenzBifurcation (), BorderLayout.CENTER);
