...
public class Coupled3BVPBifurcation extends JPanel 
{
	// instance variables
	private static double y0 = -0.6;
	private static double yf = 0.4;
	private static double x0 = 0.4;
	private static double xf = 1.8;
	...
	private double u0 = x0;
	private double us = 0.0;
	private static double eps = 0.190;
	
	// Constructor
	public Coupled3BVPBifurcation ()
	{
		...
		SwingUtilities.invokeLater(new Runnable () {
			public void run () {
				...
				for (int i = 0; i < xy.length; ++i) {
					...
					// initial value
					double v0 [] = new double [] {0.23, -0.3, 0.0};
					
					// let go of the transient values
					for (int j = 0; j < 10000; ++j)
						v0 = rungeKutta (v0);
					
					xy [i] = getMinima (new double []{v0[0], v0[1], 0.0});
				}
				drawBif = true;
				repaint ();
			}
	...
	private double [] f (double [] v)
	{
		...
		vv[0] = v[0] * (1 - eps) - Math.pow(v[0], 3) / 3 - v[1] +
				us + u0 * Math.cos(2 * Math.PI * fn * v[2]);
		vv[1] = c * (v[0] + a - b * v[1]);
		vv[2] = 1.0;
		...
		SwingUtilities.invokeLater (new Runnable () {
			public void run () {
				...
				JFrame frame = new JFrame (
						"Kyprianidis Bifurcation Diagram for epsilon = " + eps);
				frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
				frame.add(new Coupled3BVPBifurcation (), BorderLayout.CENTER);
