...
public class StabilityTest extends javax.swing.JPanel
{
	// instance variables
	...
	// constructor
	public StabilityTest (javax.swing.JFrame frame,
			...
			public void mouseClicked (java.awt.event.MouseEvent evt) {
				...
				executor.execute(new Runnable () {
					public void run () {
						...
						String [] h = getEigenValues (new double [] [] {{g, -1.0}, {1.0, -k}});
						System.out.printf("\nh1 = %s\nh2 = %s\n", h[0], h[1]);
						*/
						solve (g, k);
					}
	...
	private void solve (double g, double k)
	{
		...
		// get the fixed points for this parameter values
		java.util.ArrayList<Point> fixedPoint = getFixedPoint(g, k);
		
		// for each of the fixed point, determine the linear stability
		for (int i = 0; i < fixedPoint.size(); ++i) {
			Point p = fixedPoint.get(i);
			
			/* set the initial point about the fixed point
			// i.e. v = v' + e where v' is the fixed point and e, a small variation
			double [] v = new double [] {p.getX(), p.getY(), g / sq(Math.cosh(g * p.getX())),
					-1.0, 1.0, -k};
			...
			*/
			...
			// get eigen values of Df(v') directly
			String [] h = getEigenValues (new double [] [] {
					{g / sq(Math.cosh(g * p.getX())), -1.0}, {1.0, -k}});
			System.out.printf("\n%d. %s\th1 = %s\th2 = %s", 
					i + 1, p.toString(), h[0], h[1]);
		}
	}
	...
	private double [] f (double g, double k, double [] v)
	{
		...
		vv[2] = g / sq(Math.cosh(g * v[0])) * v[2] - v[3];
		vv[3] = v[2] - k * v[3];
		
		vv[4] = g / sq(Math.cosh(g * v[0])) * v[4] - v[5];
		vv[5] = v[4] - k * v[5];
	...
	public static void main(String[] args) {
		...
		frame.add(new StabilityTest (frame, x0, xf, y0, yf));
